...
export default function DashboardPage() {
  ...
  return (
    <main className="h-screen flex flex-col bg-neutral-50">
      {/* 데모 모드 배너 - 미니멀 */}
      {isDemo && showBanner && (
        <div className="bg-neutral-900 text-neutral-300 px-6 py-2 text-sm flex items-center justify-between">
          <div className="flex items-center gap-4">
            <span>Demo Mode</span>
            <span className="text-neutral-500">|</span>
            <span className="text-neutral-400">{SAMPLE_BIDS.length} records</span>
          </div>
          <div className="flex items-center gap-4">
            <Link href="/signup" className="text-white font-medium hover:underline">
              Get Started
            </Link>
            <button onClick={() => setShowBanner(false)} className="text-neutral-500 hover:text-white">
              ×
            </button>
          </div>
        </div>
      ...
      {/* 헤더 - 반응형 */}
      <header className="bg-white border-b border-neutral-200 px-4 md:px-6 h-14 flex items-center justify-between">
        <div className="flex items-center gap-4 md:gap-6">
          <Link href="/" className="flex items-center gap-2">
            <div className="w-7 h-7 bg-neutral-900 rounded flex items-center justify-center">
              <span className="text-white text-xs font-bold">B</span>
            </div>
            <span className="text-base font-semibold text-neutral-900 hidden sm:inline">BIDFLOW</span>
          </Link>

          {/* 데스크톱 네비게이션 */}
          <nav className="hidden md:flex items-center gap-1">
            <Link href="/dashboard" className="px-3 py-1.5 text-sm font-medium text-neutral-900 bg-neutral-100 rounded">
              Bids
            </Link>
            <a href="#" className="px-3 py-1.5 text-sm text-neutral-600 hover:text-neutral-900 rounded hover:bg-neutral-100">
              Pipeline
            </a>
            <a href="#" className="px-3 py-1.5 text-sm text-neutral-600 hover:text-neutral-900 rounded hover:bg-neutral-100">
              Analytics
            </a>
            <Link href="/docs" className="px-3 py-1.5 text-sm text-neutral-600 hover:text-neutral-900 rounded hover:bg-neutral-100">
              Docs
            </Link>
          </nav>
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          {isDemo ? (
            <Link
              href="/signup"
              className="px-3 md:px-4 py-1.5 text-sm font-medium text-white bg-neutral-900 rounded hover:bg-neutral-800"
            >
              Sign up
            </Link>
          ) : (
      ...
      {/* 통계 바 - 반응형 */}
      <div className="bg-white border-b border-neutral-200 px-4 md:px-6 py-3 overflow-x-auto">
        <div className="flex items-center gap-4 md:gap-6 min-w-max md:min-w-0">
          ...
          <div className="ml-auto flex items-center gap-2 flex-shrink-0">
            <span className="text-xs text-neutral-500 hidden sm:inline">Est. Total</span>
            <span className="text-base md:text-lg font-mono font-semibold text-neutral-900">
